salsa20 = function() {};

salsa20.crypto_core_salsa20_ref_OUTPUTBYTES = 64;
salsa20.crypto_core_salsa20_ref_INPUTBYTES = 16;
salsa20.crypto_core_salsa20_ref_KEYBYTES = 32;
salsa20.crypto_core_salsa20_ref_CONSTBYTES = 16;
salsa20.crypto_stream_salsa20_ref_KEYBYTES = 32;
salsa20.crypto_stream_salsa20_ref_NONCEBYTES = 8;

salsa20.ROUNDS = 20;

salsa20.rotate = function(u, c)
{
	return (u << c) | (u >>> (32 - c));
};

salsa20.load_littleendian = function(x, offset)
{
	return (parseInt(x[offset])&0xff) |
			(((parseInt(x[offset + 1])&0xff)) << 8) |
			(((parseInt(x[offset + 2])&0xff)) << 16) |
			(((parseInt(x[offset + 3])&0xff)) << 24);
};

salsa20.store_littleendian = function(x, offset, u)
{
	x[offset] = toByte(u); u >>>= 8;
	x[offset + 1] = toByte(u); u >>>= 8;
	x[offset + 2] = toByte(u); u >>>= 8;
	x[offset + 3] = toByte(u);
};

salsa20.crypto_core = function(outv, inv, k, c)
{
	var x0, x1, x2, x3, x4, x5, x6, x7, x8, x9, x10, x11, x12, x13, x14, x15;
	var j0, j1, j2, j3, j4, j5, j6, j7, j8, j9, j10, j11, j12, j13, j14, j15;
	var i;

	j0 = x0 = this.load_littleendian(c, 0);
	j1 = x1 = this.load_littleendian(k, 0);
	j2 = x2 = this.load_littleendian(k, 4);
	j3 = x3 = this.load_littleendian(k, 8);
	j4 = x4 = this.load_littleendian(k, 12);
	j5 = x5 = this.load_littleendian(c, 4);
	j6 = x6 = this.load_littleendian(inv, 0);
	j7 = x7 = this.load_littleendian(inv, 4);
	j8 = x8 = this.load_littleendian(inv, 8);
	j9 = x9 = this.load_littleendian(inv, 12);
	j10 = x10 = this.load_littleendian(c, 8);
	j11 = x11 = this.load_littleendian(k, 16);
	j12 = x12 = this.load_littleendian(k, 20);
	j13 = x13 = this.load_littleendian(k, 24);
	j14 = x14 = this.load_littleendian(k, 28);
	j15 = x15 = this.load_littleendian(c, 12);

	for (i = this.ROUNDS; i > 0; i -= 2)
	{
		x4 ^= this.rotate(x0 + x12, 7);
		x8 ^= this.rotate(x4 + x0, 9);
		x12 ^= this.rotate(x8 + x4, 13);
		x0 ^= this.rotate(x12 + x8, 18);
		x9 ^= this.rotate(x5 + x1, 7);
		x13 ^= this.rotate(x9 + x5, 9);
		x1 ^= this.rotate(x13 + x9, 13);
		x5 ^= this.rotate(x1 + x13, 18);
		x14 ^= this.rotate(x10 + x6, 7);
		x2 ^= this.rotate(x14 + x10, 9);
		x6 ^= this.rotate(x2 + x14, 13);
		x10 ^= this.rotate(x6 + x2, 18);
		x3 ^= this.rotate(x15 + x11, 7);
		x7 ^= this.rotate(x3 + x15, 9);
		x11 ^= this.rotate(x7 + x3, 13);
		x15 ^= this.rotate(x11 + x7, 18);
		x1 ^= this.rotate(x0 + x3, 7);
		x2 ^= this.rotate(x1 + x0, 9);
		x3 ^= this.rotate(x2 + x1, 13);
		x0 ^= this.rotate(x3 + x2, 18);
		x6 ^= this.rotate(x5 + x4, 7);
		x7 ^= this.rotate(x6 + x5, 9);
		x4 ^= this.rotate(x7 + x6, 13);
		x5 ^= this.rotate(x4 + x7, 18);
		x11 ^= this.rotate(x10 + x9, 7);
		x8 ^= this.rotate(x11 + x10, 9);
		x9 ^= this.rotate(x8 + x11, 13);
		x10 ^= this.rotate(x9 + x8, 18);
		x12 ^= this.rotate(x15 + x14, 7);
		x13 ^= this.rotate(x12 + x15, 9);
		x14 ^= this.rotate(x13 + x12, 13);
		x15 ^= this.rotate(x14 + x13, 18);
	}

	x0 += j0;
	x1 += j1;
	x2 += j2;
	x3 += j3;
	x4 += j4;
	x5 += j5;
	x6 += j6;
	x7 += j7;
	x8 += j8;
	x9 += j9;
	x10 += j10;
	x11 += j11;
	x12 += j12;
	x13 += j13;
	x14 += j14;
	x15 += j15;

	this.store_littleendian(outv, 0, x0);
	this.store_littleendian(outv, 4, x1);
	this.store_littleendian(outv, 8, x2);
	this.store_littleendian(outv, 12, x3);
	this.store_littleendian(outv, 16, x4);
	this.store_littleendian(outv, 20, x5);
	this.store_littleendian(outv, 24, x6);
	this.store_littleendian(outv, 28, x7);
	this.store_littleendian(outv, 32, x8);
	this.store_littleendian(outv, 36, x9);
	this.store_littleendian(outv, 40, x10);
	this.store_littleendian(outv, 44, x11);
	this.store_littleendian(outv, 48, x12);
	this.store_littleendian(outv, 52, x13);
	this.store_littleendian(outv, 56, x14);
	this.store_littleendian(outv, 60, x15);

	return 0;
};

salsa20.crypto_stream = function(c, clen, n, noffset, k)
{
	var inv = new Array(16);
	var block = new Array(64);
	
	var coffset = 0;
	
	if (clen == 0)
		return 0;

	for (var i = 0; i < 8; ++i)
		inv[i] = n[noffset + i];
	
	for (var i = 8; i < 16; ++i)
		inv[i] = 0;

	while (clen >= 64)
	{		
		salsa20.crypto_core(c, inv, k, xsalsa20.sigma);

		var u = 1;
		
		for (var i = 8; i < 16; ++i)
		{
			u += inv[i]&0xff;
			inv[i] = toByte(u);
			u >>>= 8;
		}

		clen -= 64;
		coffset += 64;
	}

	if (clen != 0)
	{			
		salsa20.crypto_core(block, inv, k, xsalsa20.sigma);
		
		for (var i = 0; i < clen; ++i)
			c[coffset + i] = block[i];
	}
	
	return 0;
};

salsa20.crypto_stream_xor = function(c, m, mlen, n, noffset, k)
{
	var inv = new Array(16);
	var block = new Array(64);
	
	var coffset = 0;
	var moffset = 0;
	
	if (mlen == 0)
		return 0;

	for (var i = 0; i < 8; ++i)
		inv[i] = n[noffset + i];
	
	for (var i = 8; i < 16; ++i)
		inv[i] = 0;

	while (mlen >= 64)
	{			
		salsa20.crypto_core(block, inv, k, xsalsa20.sigma);
		
		for (var i = 0; i < 64; ++i)
			c[coffset + i] = toByte(m[moffset + i] ^ block[i]);

		var u = 1;
		
		for (var i = 8; i < 16; ++i)
		{
			u += inv[i]&0xff;
			inv[i] = toByte(u);
			u >>>= 8;
		}

		mlen -= 64;
		coffset += 64;
		moffset += 64;
	}

	if (mlen != 0)
	{
		salsa20.crypto_core(block, inv, k, xsalsa20.sigma);
		
		for (var i = 0; i < mlen; ++i)
			c[coffset + i] = toByte(m[moffset + i] ^ block[i]);
	}
	
	return 0;
};